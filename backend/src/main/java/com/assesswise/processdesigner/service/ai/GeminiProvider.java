 ...
 * analysis schema beyond the optional server-side response schema hint.
 */
@Component
...
public class GeminiProvider implements AiProvider {
    ...
    public GeminiProvider(AppProperties properties, ObjectMapper objectMapper) {
        ...
        if (!config.isConfigured()) {
            log.warn("GEMINI_API_KEY is not set — process CRUD works, but POST /analyze will return 503 "
    ...
    private String handleResponse(org.springframework.http.client.ClientHttpResponse httpResponse) throws IOException {
        String payload = new String(httpResponse.getBody().readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
    ...
    private AiCompletion parseCompletion(String responseBody, long durationMs, String purpose) {
        ...
        return new AiCompletion(text.toString(), promptTokens, outputTokens, durationMs, finishReason);
    }
