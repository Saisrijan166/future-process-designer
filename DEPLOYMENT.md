...
| An analysis stops part-way and the run shows as failed, with `Commencing graceful shutdown` in the log seconds earlier | The instance was killed mid-run, so nothing failed — the JVM holding the run went away, and the startup sweep marked the orphaned run failed on the next boot | Render's **Events** tab names the reason; the service log never will. On a free instance it is usually memory, and the JVM ceilings in `backend/Dockerfile` are budgeted to fit 512 MB. If Events says a deploy or a restart instead, the run was collateral damage from that |
